...
public class ArrayTaskList {
    ...
    public boolean remove(Task task){
        ...
        for (Task temp : arrayTask){
               if(task == temp){                                    //Sí la tarea obtenida del arreglo es iigual a la tarea que busco.
    ...
    public Task getTask(int index){
        if(index > this.index || index <0){                                                         //Comprobando que el índice ingresado sea válido.
            throw new IndexOutOfBoundsException("Index: "+this.index+", Size: "+arrayTask.length);
        }else{
